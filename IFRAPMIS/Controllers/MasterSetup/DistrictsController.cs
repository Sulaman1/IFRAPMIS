...
namespace IFRAPMIS.Controllers.MasterSetup
{
    public class DistrictsController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("DistrictId, DistrictName, Code, DivisionId")] District district)
        {
            if (ModelState.IsValid)
            {
                var IsExist = _context.Count(district.DistrictName);
                if (IsExist > 0)
                {
                    ModelState.AddModelError(nameof(district.DistrictName), "Already exist with same name!");
                    return View(district);
                }
